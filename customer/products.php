...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['negotiate'])) {
    $pid=(int)$_POST['neg_pid']; $qty=(int)$_POST['neg_qty'];
    $offer=(float)$_POST['offered_price']; $nmsg=$conn->real_escape_string(trim($_POST['neg_msg']));
    $p=$conn->query("SELECT * FROM products WHERE product_id=$pid LIMIT 1")->fetch_assoc();
    if ($p) {
        $sid=(int)$p['seller_id'];
        $ex=$conn->query("SELECT neg_id FROM negotiations WHERE product_id=$pid AND customer_id=$uid AND status='pending' LIMIT 1");
        if ($ex->num_rows>0) $msg="<div class='alert alert-warning'>You already have a pending negotiation for this product.</div>";
        else {
            $conn->query("INSERT INTO negotiations(product_id,customer_id,seller_id,quantity,offered_price,message) VALUES($pid,$uid,$sid,$qty,$offer,'$nmsg')");
            addNotification($sid,"New bulk price negotiation for '{$p['name']}'");
...
<div class="main">
  <div class="topbar"><div class="topbar-title">Browse Products</div>
    <div class="topbar-actions">
      <a href="/oil_supply/customer/negotiations.php" class="btn btn-outline btn-sm">🤝 My Negotiations</a>
