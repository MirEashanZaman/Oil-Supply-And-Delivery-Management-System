...
$conn=getDB();
$ps=$conn->real_escape_string($_GET['product']??'Industrial Lubricant Oil');
$tr=$conn->query("SELECT DATE_FORMAT(recorded_at,'%b %Y') mon,ROUND(AVG(price),2) ap FROM market_prices WHERE product_name='$ps' GROUP BY DATE_FORMAT(recorded_at,'%Y-%m') ORDER BY recorded_at ASC LIMIT 6");
$tl=[]; $td=[]; while($r=$tr->fetch_assoc()){$tl[]=$r['mon'];$td[]=(float)$r['ap'];}
$reg=$conn->query("SELECT region,MIN(price) low,MAX(price) high FROM market_prices WHERE product_name='$ps' GROUP BY region");
$lu=$conn->query("SELECT MAX(recorded_at) mx FROM market_prices")->fetch_assoc()['mx'];
$pnames=$conn->query("SELECT DISTINCT product_name FROM market_prices ORDER BY product_name");
$conn->close();
?><!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Market Prices</title><link rel="stylesheet" href="/oil_supply/css/style.css"><script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.min.js"></script></head><body><div class="app">
<?php require_once __DIR__.'/../includes/sidebar.php'; ?>
<div class="main">
  <div class="topbar"><div class="topbar-title">Market Transparency</div><div class="topbar-actions"><span style="font-family:'Share Tech Mono',monospace;font-size:.65rem;color:var(--muted);">Last Updated: <?=date('M d, Y',strtotime($lu??'now'))?></span></div></div>
  <div class="content">
